...
class firebaseFunictions {
  ...
  static Stream<QuerySnapshot<TaskModel>> getTask(DateTime dateTime){
    return getTasksCollection().where("UserId",isEqualTo: FirebaseAuth.instance.currentUser!.uid).where("date",isEqualTo: DateUtils.dateOnly(dateTime).millisecondsSinceEpoch)
        .snapshots();
  }
  ...
  static Future<void>Signup(String name,String email,String password,int phoneNum,Function Nav)async{
    try {
      var credential = await FirebaseAuth.instance.createUserWithEmailAndPassword(
        email: email,
        password: password,).then((credential){
        Nav();
          UserModel userModel=UserModel(id:credential.user!.uid ,name: name, phoneNum: phoneNum, Email: email);
